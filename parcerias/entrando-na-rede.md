...
**Onde fica:** no espaço **Rede de Parceiros** — o alternador no menu lateral (ou na aba Menu) que troca da sua Operação para a Rede, com as abas **Início · Descobrir · Acordos · Mais**. É um espaço próprio, com o seu acento visual, porque parceria é um assunto diferente do dia a dia da locação: aqui você não atende cliente, você se conecta a outras empresas.
...
O caminho: **Rede › Parceiros**, botão de **convidar**. A tela **Convidar parceiro externo** pede só o essencial:

1. **Nome** do parceiro (ex.: *Márcia Fretes*).
2. **E-mail (opcional)** — se informar, o convite também chega por e-mail.
3. Toque em **Convidar** → o LocFlow gera um **link de convite**.
...
{% hint style="success" %}
**Por que isso é seguro:** o parceiro externo nunca vê os seus clientes, os seus preços ao cliente nem os outros orçamentos. Ele vê a operação que é dele e **quanto ele ganha** — o suficiente para trabalhar, nada além.
{% endhint %}

Depois do convite aceito, faltam dois cadastros para o parceiro ficar operacional — o **galpão** dele (a origem do frete) e o **recebedor** de pagamento. O passo a passo completo, incluindo o que ele enxerga por dentro, está em [Parceiro Logístico Externo](parceiro-logistico-externo.md).
...
{% hint style="info" %}
Propor parceria **não cria compromisso nenhum** — nem preço, nem estoque, nem obrigação. O vínculo é só o aperto de mão: "nossas organizações se conhecem e podem negociar". Os termos de verdade vêm depois, no **acordo**.
{% endhint %}
...
| **Encerrado** | A conexão foi desfeita | Nada — mas um novo vínculo pode ser proposto no futuro. |

O vínculo **ativo** é a chave que abre a porta dos [acordos de parceria](acordos-de-parceria.md): itens mapeados entre os dois catálogos, gatilho de pagamento do repasse, janelas de aceite e desistência. Sem vínculo ativo, não há acordo interno.

**Encerrar** um vínculo pede confirmação e desfaz a conexão — use quando a parceria de fato acabou, não como pausa: os acordos dependem dele.
